import itertools

table = [
    0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
    0x39, 0x30, 0x71, 0x77, 0x65, 0x72, 0x74, 0x79,
    0x75, 0x69, 0x6F, 0x70, 0x61, 0x73, 0x64, 0x66,
    0x67, 0x68, 0x6A, 0x6B, 0x6C, 0x7A, 0x78, 0x63,
    0x76, 0x62, 0x6E, 0x6D, 0x51, 0x57, 0x45, 0x52,
    0x54, 0x59, 0x55, 0x49, 0x4F, 0x50, 0x41, 0x53,
    0x44, 0x46, 0x48, 0x4A, 0x4B, 0x4C, 0x5A, 0x58,
    0x43, 0x56, 0x42, 0x4E, 0x4D, 0x2D, 0x2B, 0x2F
]

result = 'pL97'
characters = '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ'

for sb in itertools.product(characters, repeat=4):
    sb = ''.join(sb)
    v = [0]*16
    v11 = 8
    v[v11] = table[(ord(sb[0]) >> 2) & 0x3F]
    v[v11 + 1] = table[((ord(sb[1]) >> 4) & 0xF) | ((ord(sb[0]) << 4) & 0x30)]
    v[v11 + 2] = table[((ord(sb[2]) >> 6) & 0x3) | ((ord(sb[1]) << 2) & 0x3C)]
    v[v11 + 3] = table[(ord(sb[3]) ^ ord(sb[2])) & 0x3F]
    computed = ''.join(chr(v[i]) for i in range(v11, v11 + 4))
    if computed == result:
        print(sb)
